...
export default function ProjectDetails() {
  ...
  return (<div className="bg-indigo-950 h-screen grid items-center">
    <motion.section className="p-10 max-w-3xl mx-auto"
          initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      transition={{ duration: 1, delay: 0.1 }}>
      <h2 className="text-3xl text-cyan-300 font-bold">{project.name}</h2>
      <p className="mt-2">{project.description}</p>

      <h4 className="mt-4 font-bold">Tech Stack</h4>
      <ul>{project.tech.map(t => <li className="list-disc list-inside" key={t}>{t}</li>)}</ul>
